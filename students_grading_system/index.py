    ...
    if menu == "teacher":
        while True:
            ...
            match task:
                ...
                case _:
                    print(
                        f"{task} operation is either not on this system or not within your juristiction"
                    )
...
def create_teacher():
    ...
    if class_details:
        class_details["teacher"] = teach_name.capitalize()
...
def delete_teacher():
    teacher_name = input("Teacher Name: ").strip().capitalize()
    teacher_id = input("Teacher ID: ").strip()
    class_name, class_details = find_class_by_condition(
        lambda name, detials: detials["teacher_id"] == teacher_id
        and detials["teacher"] == teacher_name
    )

    print(class_name, class_details)
    if class_details:
        class_details["teacher"] = ""
        class_details["teacher_id"] = ""
        save_to_file()
        print(f"{teacher_name} with ID of {teacher_id} has been removed from system")
    else:
    ...
    if class_details:
        ...
        if student is not None:
            try:
                ...
                student["teacher_remark"] = input("Your remark on this student: ")
            ...
            while True:
                ...
                try:
                    ...
                    if score < 0 or score > SCORE_LIMITS[criteria]:
                        print(
                            f"{criteria} score is in range of 1 to {SCORE_LIMITS[criteria]}"
                        )
                        continue
...
def generate_report_card(student):
    ...
    CLASS: {student["student class"].title()}
    ID:    {student["student_id"]}
